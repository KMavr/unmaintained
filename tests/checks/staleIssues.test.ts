...
describe('staleIssuesCheck', () => {
  it('should flag an old repo with a high open-issue ratio above the floor', () => {
    const reason = staleIssuesCheck(base, now);
  ...
  it('should return null when issue counts are unknown (no token)', () => {
    expect(staleIssuesCheck({ ...base, openIssues: null }, now)).toBeNull();
    expect(staleIssuesCheck({ ...base, closedIssues: null }, now)).toBeNull();
    expect(staleIssuesCheck({ ...base, repoCreatedAt: null }, now)).toBeNull();
  });

  it('should return null for a repo younger than the age guard', () => {
    expect(staleIssuesCheck({ ...base, repoCreatedAt: '2026-03-01T00:00:00Z' }, now)).toBeNull();
  });

  it('should return null when total issues are below the floor', () => {
    expect(staleIssuesCheck({ ...base, openIssues: 9, closedIssues: 1 }, now)).toBeNull();
  });

  it('should return null when the open ratio is below the threshold', () => {
    expect(staleIssuesCheck({ ...base, openIssues: 10, closedIssues: 10 }, now)).toBeNull();
  });
});
